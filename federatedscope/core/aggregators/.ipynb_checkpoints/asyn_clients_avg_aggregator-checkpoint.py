...
class AsynClientsAvgAggregator(ClientsAvgAggregator):
    ...
    def aggregate(self, agg_info):
        ...
        for x in agg_info['staleness']:
            if x[0] not in self.count:
                self.count[x[0]]=1
            else:
                self.count[x[0]]=+1
        frequency=[1/self.count[x[0]] for x in agg_info['staleness']]
        ...
        self.parameter.append(updated_model)
        if len(self.parameter)>10:
    ...
    def _para_weighted_avg(self, models, recover_fun=None, staleness=None,frequency=None):
        """
        ...
        for key in avg_model:
            for i in range(len(models)):
                ...
                weight *= frequency[i]*len(frequency)/sum(frequency)
                ...
                try:
                    local_model[key]+=0.1*(self.parameter[-1][key]-self.parameter[-1-staleness[i]][key])
                except:
                    pass
                if i == 0:
