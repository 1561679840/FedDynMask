...
class AdapterModel(nn.Module):
    ...
    """
    def __init__(self, model, use_adapter=False, use_SVD=False, *args, **kwargs):
        """
        ...
        if use_adapter:
            ...
            if use_SVD:
                ...
                for name,param in self.model.named_parameters():
                    if "lora_A" in name:
                        r=param.shape[0]
                        U, S, Vh = torch.linalg.svd(weight[name.replace('base_model.model.','').replace('lora_A.default','base_layer')], full_matrices=False)
