...
class LLMTrainer(GeneralTorchTrainer):
        ...
        if ctx.cur_mode == MODE.TEST and ctx.cfg.data.type=="gsm8k@llm":
            ...
            for batch in ctx.val_loader_copy:
                for instruction, _, output in zip(batch['instruction'], batch['input'], batch['output']):
                    ...
                    generate_kwargs = dict(max_new_tokens=256, top_p=0.95, temperature=0.5)
    ...
    def update(self, model_parameters, strict=False):
        ...
        """
        if self.cfg.federate.sample_client_rate>0 and self.cfg.federate.sample_client_rate<1:
            self.round=math.floor(len(LLMTrainer.cluster)/(self.cfg.federate.client_num*self.cfg.federate.sample_client_rate))
        ...
        if self.ctx.cfg.train.sparse_B == True:
            if self.round < 10:
                ...
                if self.cfg.federate.sample_client_rate>0 and self.cfg.federate.sample_client_rate<1:
                    rate=self.cfg.federate.sample_client_rate
                else:
                    rate=1.0
                if len(LLMTrainer.clients_Mask)==(self.cfg.federate.client_num*rate):
                ...
                for n,p in self.ctx.model.named_parameters():
                    if 'lora_B' in n:
                        try:
                            ...
                            model_parameters[n[6:]]=model_parameters[n[6:]]*LLMTrainer.global_Mask[n[6:]]
                LLMTrainer.clients_Mask.append(cfg_mask)
                v=0
                p=0
                if self.cfg.federate.sample_client_rate>0 and self.cfg.federate.sample_client_rate<1:
        ...
        self.S_model=0
        self.round+=1
        LLMTrainer.cluster.append(self.round)
