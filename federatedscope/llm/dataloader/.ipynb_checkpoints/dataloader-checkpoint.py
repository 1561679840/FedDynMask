...
def get_tokenizer(model_name, cache_dir, tok_len=128, pkg='huggingface_llm'):
    ...
    path='../cache/hub/models/'+ model_name
    print(path)
...
def load_llm_dataset(config=None, **kwargs):
    ...
    if dataset_name.endswith('.json'):
        ...
        dataset = LLMDataset(list_data_dict, tokenizer)
    elif dataset_name.lower() == 'multi':
        fp = os.path.join(config.data.root, 'multi.jsonl')
        list_data_dict = load_jsonl(fp,
                                    instruction='instruction',
                                    input='input',
                                    output='output',
        ...
        dataset = LLMDataset(list_data_dict, tokenizer)
    elif dataset_name.lower()=='nat':
        fp = os.path.join(config.data.root, 'nat.jsonl')
        list_data_dict = load_jsonl(fp,
                                    instruction='instruction',
                                    input='input',
                                    output='output',
        ...
        dataset = LLMDataset(list_data_dict, tokenizer)
        list_data_dict = [
                i for i in list_data_dict if i['output'] != ''
            ]
    else:
